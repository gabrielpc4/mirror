	...
	: Sprite(startX, startY)
{
	...
	Sprite::push_back(Rect(Point(0, 0), Point(43, 30)), Color(167.0, 0.0, 0.0));   // DarkRed rectangle: width:stairs.width() height: 12 - 6 = 6	
	tunnelHole = Rect(Point(0, 0), Point(this->width(), 30));
	cover.push_back(Rect(Point(startX, startY + 30), Point(startX + this->width(), startY + 40)), Color(287, 147, 0));
}
